...
f=glob.glob(os.path.join(folder,'*.mzML')) #SS_170615
...
def calcRoots(pfun,y0,miny,maxy):
    ...
    try:
        ...
        for oneRoot in myRoots:
            if np.iscomplex(oneRoot):
                toohigh.append(np.Inf)
            elif oneRoot<0:
                negatives.append(oneRoot)
            elif oneRoot>2*maxy:
                toohigh.append(oneRoot)
            elif oneRoot >=miny and oneRoot<=maxy:
                possibles.append(oneRoot)
            elif oneRoot<miny:
                lessThan.append(oneRoot)
            elif oneRoot>maxy:
                moreThan.append(oneRoot)
        ...
        if len(possibles)==1:
            ...
            if len(lessThan)>0 and len(moreThan)>0:
                return str(round(max(lessThan),3))+'<LLOQ:'+str(miny)+'&'+str(round(min(moreThan),1))+' >ULOQ:'+str(maxy)
            elif len(lessThan)>0:
                return str(round(max(lessThan),3))+'<LLOQ:'+str(miny)
            elif len(moreThan)>0:
                return str(min(moreThan))+'>ULOQ:'+str(maxy)
            elif len(negatives)==2:
                return 'neg<0:'
            elif len(toohigh)==2:
                return 'pos>2*ULOQ:'
            elif len(toohigh)==1 and len(negatives)==1:
                return 'neg<0'+'&'+'pos>2*ULOQ:'
            else:
                return '??,'+','.join((str(myRoots[0]),str(myRoots[1])))
    except TypeError:
        return 'N/A'
    return 'N/A'
...
for oneComp in allComps2:
    ...
    poly_minmax[oneComp]=(min(conc),max(conc))
    if oneComp in ['Spermidine']:
    ...
    for fname,oneRes in ratioDict.iteritems():
        ...
        #for compound,oneRes2 in oneRes.iteritems():
                ...
                if oneComp in mqResConc[fname]:
                    ...
                    mqr=calcRoots(poly_mq[oneComp],mqRes[fname][oneComp],poly_minmax[oneComp][0],poly_minmax[oneComp][1])
                    bigr=calcRoots(poly_bigauss[oneComp],oneRes2['bigauss'],poly_minmax[oneComp][0],poly_minmax[oneComp][1])
                    gaur=calcRoots(poly_gauss[oneComp],oneRes2['gauss'],poly_minmax[oneComp][0],poly_minmax[oneComp][1])
                    #print gaur,bigr,mqr
                    f.write("\t")
